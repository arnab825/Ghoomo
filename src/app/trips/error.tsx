...
export default function TripsError({
  ...
  React.useEffect(() => {
    console.error('[TripsError] Route caught exception:', error);
  }, [error]);
      ...
      <div className="max-w-md w-full p-8 rounded-lg border border-red-200 bg-white dark:border-rose-900/50 dark:bg-slate-900 shadow-md text-center space-y-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white font-heading">
            Unable to Load Trips Library
          </h2>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1.5 leading-relaxed">
            {error.message ||
              'We were unable to retrieve your trip library. Please verify your connection or try again.'}
          </p>
        </div>
